...
/**
 * 吧台凳方块实体 管理坐骑 ArmorStand 的生成/回收，每 tick 跟随玩家视角转动并持久化 last_yaw
 * 交互入口见 {@link BarStoolBehavior}，渲染发包见 {@link BarStoolElement}。
 */
public class BarStoolController extends BlockEntityController {
    private static final String CHAIR_TAG = "nico_stool_sit";

    // TODO 硬编码的座椅高度，若需要添加不同高度的座椅，这里修改一下就好，暂时应该不会
    ...
    public void tick() {
        if (needsRecoveryCheck) {
            ...
            Location checkLoc = new Location(
                    (org.bukkit.World) super.blockEntity.world.world().platformWorld(),
