...
public final class PotElement implements BlockEntityElement {
    ...
    @Override
    public void activate() { refreshPackets(); }

    @Override
    public void deactivate() { cancelAnimationTasks(); }
    ...
    public void showIndex(Player player, int index) { display.showSlot(player, index); }

    public void hideIndex(Player player, int index) {
        if (index >= 0 && index < SLOTS) display.removeSlot(player, index);
    }

    public void hideAll(Player player) { display.removeAll(player); }

    // 食材烧焦
    public void updateBrightness(Player player, int brightness) {
        if (controller.stage() != CookingStage.BURNT) return;
    ...
    public void refreshSlotPacket(int index) {
        ...
        if (controller.stage() == CookingStage.BURNT) packets.brightness(burntBrightness(controller.getCurrentTick(), controller.burntToCharcoalTime()));
    ...
    public void playStirFryAnimation(Runnable onComplete) {
        ...
        for (int step = 1; step <= 8; step++) {
            ...
            Runnable phaseRunnable = () -> {
                ...
                chunk.getTrackedBy().forEach(p -> p.sendPacket(bundle, false));
            };
