...
/**
 * 搪瓷盆方块实体 维护油量/开合状态，按油量刷新方块状态并负责 NBT 存读、移除掉落
 * 交互入口见 {@link EnamelBasinBehavior}
 */
public class EnamelBasinController extends BlockEntityController {
    ...
    public void addOil(int amount) {
        if (oilCount >= behavior.maxOil) return;
    ...
    // TODO: 裸露了油量后这里应该动态显示，等提出来就改
    private int getOilLevelInt(int oilCount) {
        return switch (oilCount) {
            case 0 -> 1; // empty
            case 1, 2, 3, 4, 5 -> 2; // low
            case 6, 7, 8, 9, 10 -> 3; // middle
            default -> 4; // high
        };
    }
