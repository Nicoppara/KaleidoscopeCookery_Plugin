...
/**
 * 烤肉串方块行为 处理右键放料/取料，红石信号控制旋转，上下两半联动
 * 数据见 {@link ShawarmaSpitController}，渲染见 {@link ShawarmaSpitElement}
 *
 * 可在 behaviors 配置里覆盖（默认值即下方所示，键名支持下划线/连字符）
 * behaviors:
 *   - type: nicopengren:shawarma_spit
...
public final class ShawarmaSpitBehavior extends BukkitBlockBehavior implements EntityBlock {
    ...
    public InteractionResult useOnBlock(UseOnContext context, ImmutableBlockState state) {
        ...
        if (!itemInHand.isEmpty()) {
            if (!controller.canCook(itemInHand)) return InteractionResult.PASS;
            int held = itemInHand.count();
            int placed = 0;
            while (placed < held && controller.tryAddOne(layer, itemInHand)) placed++;
            if (placed > 0) {
    ...
    private void setHalfPowered(Object world, Object pos, boolean powered) {
        ...
        if (s == null || s.owner().value() != super.blockDefinition) return;
