...
import top.nicoppa.nicoPengRen.common.block.BlockStates;
import top.nicoppa.nicoPengRen.common.item.BlockEntityNbt;
...
/**
 * 高汤锅方块实体 持有烹饪数据与状态机（汤底/食材/锅盖/成品，阶段见 {@link StockpotStage}）
 * 按 tick 推进烹饪并触发渲染刷新，负责 NBT 存读
 * 交互入口见 {@link StockpotBehavior}，渲染见 {@link StockpotElement}
 */
public class StockpotController extends BlockEntityController {
    static final int MAX_INGREDIENTS = 8;
    ...
    public void tick() {
        ...
        if (bWorld.getGameTime() % 15 == 0) {
            ...
            super.blockEntity.world.world().playSound(
                    ...
                    Key.of("custom:stockpot_" + (int) (ThreadLocalRandom.current().nextDouble() * 7)),
                    volume, pitch, net.momirealms.craftengine.core.sound.SoundSource.BLOCK);
        }
    ...
    private boolean hasHeatSource() {
        ...
        Object blockPos = net.momirealms.craftengine.bukkit.util.LocationUtils.toBlockPos(super.blockEntity.pos);
        Object belowPos = net.momirealms.craftengine.bukkit.util.LocationUtils.below(blockPos);
    ...
    public boolean addLid(Item lidItem) {
        ...
        net.momirealms.craftengine.bukkit.plugin.BukkitCraftEngine.instance().scheduler().platform()
    ...
    public Item extractSoupBase() {
        ...
        return convertSoupBaseToBucket(soupBaseId);
    }

    private Item convertSoupBaseToBucket(Key soupBaseId) {
    ...
    public StockpotStage stage() { return stage; }
    public int getCurrentTick() { return currentTick; }
    public int getTakeoutCount() { return takeoutCount; }
    public int getFinishedMax() { return finishedMax; }
    public List<Key> getLastCookedIngredients() { return lastCookedIngredientsView; }
    public List<Item> getIngredients() { return ingredientsView; }
    public Key getSoupBaseId() { return soupBaseId; }

    public Item getLidItem() { return lidItem; }
    public long getSeed() { return seed; }
    public RenderTracker renderTracker() { return renderTracker; }
}