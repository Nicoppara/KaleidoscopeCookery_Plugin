...
/**
 * 留言文本渲染元素 用一个 TextDisplay 悬浮展示留言文本，并按宽度做居中偏移补偿
 * 由 {@link MessageBoardController} 创建并驱动显隐/位移
 * TODO：正式版删除 临时项目
 */
public class MessageBoardCommentElement {
    ...
    private void refreshPackets() {
        ...
        this.updatePosPacket = net.momirealms.craftengine.bukkit.util.EntityUtils.createUpdatePosPacket(
        ...
        DisplayData.TextDisplayData.Scale.addEntityData(new org.joml.Vector3f(1f, 1f, 1f), dataValues);
        ...
        // 当最长行字符单位数 >= 10 时，才进行偏移补偿
        if (maxLineUnits >= 10) {
            float textWidth = calculateTextWidth(longestLine);
            float translationX = textWidth * 0.5f; // 向右偏移半个宽度
            DisplayData.TextDisplayData.Translation.addEntityData(
                    new org.joml.Vector3f(translationX, 0f, 0f), dataValues
            );
        }
