...
/**
 * ItemDisplay 假实体发包构建器
 * 用法 {@code ItemDisplayPackets.at(pos).item(stack).scale(0.5f).translation(0,h,0).rotation(angle).itemTransform((byte)8)}
 * 然后 {@code .spawn(id, uuid)} / {@code .meta(id)}；只更新数据时用 {@code ItemDisplayPackets.builder()...meta(id)}。
 */
