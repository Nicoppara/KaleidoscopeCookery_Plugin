package top.nicoppa.nicoPengRen.util;
...
public class RecipeUtils {
    private static final NamespacedKey HAS_RECIPE_KEY = new NamespacedKey(NicoPengRen.getPlugin(NicoPengRen.class), "has_recipe");

    public static final NamespacedKey RECIPE_ID_KEY =
            new NamespacedKey(NicoPengRen.getPlugin(NicoPengRen.class), "recipe_id");
    public static final NamespacedKey RECIPE_TYPE_KEY =
            new NamespacedKey(NicoPengRen.getPlugin(NicoPengRen.class), "recipe_type");
    public static final NamespacedKey RECIPE_INGREDIENTS_KEY =
            new NamespacedKey(NicoPengRen.getPlugin(NicoPengRen.class), "recipe_ingredients");
    ...
    public static boolean tryAutoFill(Player player, ItemStack recipeStack,
                                      java.util.function.Consumer<Item> addIngredient) {
        ...
        if (recipeIdStr == null || recipeType == null) return false;
        ...
        if (ingStr != null && !ingStr.isEmpty()) {
            needed = java.util.Arrays.stream(ingStr.split(","))
                    .map(Key::of)
                    .toList();
        } else {
            // fallback：旧数据兼容 TODO 正式版时可删除
            if ("accurate".equals(recipeType)) {
                ...
                needed = r.require().stream()
                        .flatMap(req -> java.util.Collections.nCopies(req.count(), req.item()).stream())
                        .toList();
            }
        }
    ...
    private static ItemStack findInInventory(Player player, Key key) {
        for (ItemStack item : player.getInventory().getContents()) {
            ...
            if (wrapped.id().equals(key)) return item;
        }
        return null;
    }
    ...
    public static void setRecipeItem(ItemStack stack, Key recipeId, String recipeType, List<Key> actualIngredients, Key liquid) {
        ...
        List<Key> recipeLiquids = java.util.List.of();
        ...
        if ("flex".equals(recipeType)) {
            ...
            if (res.isPresent()) {
                ...
                for (int i = 0; i < Math.max(1, fr.count()); i++) {
                    outLineBuilder.append("<image:custom:").append(img).append(">");
                }
        ...
        if (liquid != null && !liquid.toString().equals("minecraft:water")) {
            lore.add(MiniMessage.miniMessage().deserialize(""));
            lore.add(MiniMessage.miniMessage().deserialize("<white><!i>所需汤底：<image:custom:" + liquid.value() + ">"));
        }
