...
public final class ChoppingEditMenu {
   ...
   public static void open(Player bukkitPlayer, ChoppingRecipeDraft draft) {
      net.momirealms.craftengine.core.entity.player.Player viewer = RecipeMenus.adapt(bukkitPlayer);
      if (viewer != null) {
         GuiLayout layout = new GuiLayout(new String[]{"#########", "#I#T#N#M#", "#V#######", "#RRRRRRR#", "#EEEEEEE#", "B###S###D"});
         ...
         layout.addIngredient('B', MenuIcons.back(viewer, () -> RecipeListMenu.open(bukkitPlayer, ApplianceType.CHOPPING_BOARD, true)));
   ...
   private static GuiElement inputSlot(Player bukkitPlayer, net.momirealms.craftengine.core.entity.player.Player viewer, ChoppingRecipeDraft draft) {
   ...
   private static GuiElement idSlot(Player bukkitPlayer, net.momirealms.craftengine.core.entity.player.Player viewer, ChoppingRecipeDraft draft) {
      Item icon = MenuIcons.icon(
   ...
   private static GuiElement stageSlot(Player bukkitPlayer, net.momirealms.craftengine.core.entity.player.Player viewer, ChoppingRecipeDraft draft) {
      Item icon = MenuIcons.icon(
         MenuButton.COUNT,
         viewer,
      ...
      return MenuIcons.button(icon, () -> MenuInput.requestInt(bukkitPlayer, "切几刀", "次数", draft.stage(), 1, 16, value -> {
         draft.stage(value);
         open(bukkitPlayer, draft);
      }, () -> open(bukkitPlayer, draft)));
   }

   private static GuiElement modeSlot(Player bukkitPlayer, net.momirealms.craftengine.core.entity.player.Player viewer, ChoppingRecipeDraft draft) {
      Item icon = MenuIcons.icon(
         MenuButton.MODE,
         viewer,
   ...
   private static GuiElement modelSlot(Player bukkitPlayer, net.momirealms.craftengine.core.entity.player.Player viewer, ChoppingRecipeDraft draft) {
      ...
      lore.add("左键修改  右键清空");
      Item icon = MenuIcons.icon(
         prefix == null ? MenuIcons.iconKey(MenuButton.CARRIER_NONE) : MenuIcons.iconKey(MenuButton.CREATE),
         viewer,
      ...
      return GuiElement.constant(icon, (element, click) -> {
         ...
         if (!"RIGHT".equals(click.type()) && !"SHIFT_RIGHT".equals(click.type())) {
            MenuInput.requestText(bukkitPlayer, "分阶段模型前缀", "前缀", prefix == null ? "" : prefix, raw -> {
   ...
   private static Ingredient resultSlots(
      final Player bukkitPlayer, final net.momirealms.craftengine.core.entity.player.Player viewer, final ChoppingRecipeDraft draft, final boolean extra
   ) {
      return new Ingredient() {
         private int index = 0;

         public GuiElement element(Gui gui) {
            return ChoppingEditMenu.resultSlot(bukkitPlayer, viewer, draft, extra, this.index++);
         }
      };
   }

   private static GuiElement resultSlot(
      Player bukkitPlayer, net.momirealms.craftengine.core.entity.player.Player viewer, ChoppingRecipeDraft draft, boolean extra, int index
   ) {
      if (extra && draft.mode() != ChoppingMode.SINGLE_EXTRA) {
         ...
         } else if (index <= list.size() && list.size() < 7) {
            Item icon = MenuIcons.icon(
               MenuButton.ADD,
               viewer,
            ...
            return GuiElement.constant(
               ...
               (element, click) -> {
                  ...
                  AccurateEditMenu.pickItem(
                     ...
                     extra ? "添加附带产物" : "添加成品",
                     null,
                     key -> list.add(new ChoppingResult(key, 1, 100)),
                     () -> open(bukkitPlayer, draft)
                  );
               }
            );
   ...
   private static GuiElement existingResult(
      Player bukkitPlayer, net.momirealms.craftengine.core.entity.player.Player viewer, ChoppingRecipeDraft draft, List<ChoppingResult> list, int index
   ) {
      ChoppingResult result = list.get(index);
      List<Component> lore = MenuIcons.loreNamed(
         result.key(),
         "数量 " + result.count(),
      ...
      Item icon = MenuIcons.icon(result.key(), viewer, MenuIcons.itemName(result.key()).colorIfAbsent(NamedTextColor.GOLD), lore);
      return GuiElement.constant(icon, (element, click) -> {
         click.cancel();
         ...
         if ("SHIFT_RIGHT".equals(type)) {
            ...
            MenuInput.requestInt(bukkitPlayer, "权重", "值", result.weight(), 1, 100, value -> {
               list.set(index, new ChoppingResult(result.key(), result.count(), value));
               open(bukkitPlayer, draft);
            }, () -> open(bukkitPlayer, draft));
         } else {
            MenuInput.requestInt(bukkitPlayer, "数量", "值", result.count(), 1, 64, value -> {
   ...
   private static GuiElement saveSlot(Player bukkitPlayer, net.momirealms.craftengine.core.entity.player.Player viewer, ChoppingRecipeDraft draft) {
   ...
   private static GuiElement deleteSlot(Player bukkitPlayer, net.momirealms.craftengine.core.entity.player.Player viewer, ChoppingRecipeDraft draft) {
      if (draft.isNew()) {
         return MenuIcons.filler(viewer);
      }
