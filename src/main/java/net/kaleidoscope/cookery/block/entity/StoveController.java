...
public final class StoveController extends BlockEntityController {
   ...
   private void animateTick(World level, BlockPos pos, ImmutableBlockState state) {
      Property<Boolean> litProperty = this.behavior.getLitProperty();
      if (BlockStates.value(state, litProperty, false)) {
         ...
         double sx = x + random.nextDouble() / 3.0 * (random.nextBoolean() ? 1 : -1);
         double sy = y + 0.5 + random.nextDouble() / 3.0;
         double sz = z + random.nextDouble() / 3.0 * (random.nextBoolean() ? 1 : -1);
         ...
         if (facingProperty != null) {
            ...
            xOffset = axis == Axis.X ? direction.stepX() * 0.52 : offsetRandom;
            zOffset = axis == Axis.Z ? direction.stepZ() * 0.52 : offsetRandom;
         }
         ...
         int count = this.behavior.particleCount;
         FoliaUtil.run(() -> {
            if (crackle) {
