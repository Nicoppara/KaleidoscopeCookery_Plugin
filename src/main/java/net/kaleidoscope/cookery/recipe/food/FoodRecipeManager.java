...
// 食谱系统管理器：注册各类配方的配置解析器（YAML 格式见 README）
public final class FoodRecipeManager {
    ...
    // 解析并注册一条模糊配方(pot/stock 共用)；非法(require 超过 raw min)则告警跳过，返回是否注册成功
        ...
        for (ItemRequirement req : require) {
            for (RawRequirement r : raw) {
                if (r.min() > 0 && cat.isInCategory(cook, req.item(), r.category()) && r.min() < req.count()) {
                    KaleidoscopeCookeryPlugin.getPlugin(KaleidoscopeCookeryPlugin.class).getLogger().warning(
